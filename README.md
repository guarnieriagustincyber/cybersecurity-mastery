# 🛡️ Cybersecurity Mastery

## Formación Integral en Ciberseguridad

**Enfoque:** Sin resúmenes, sin atajos, sin librerías mágicas  
**Duración:** 9 meses (Mar 2026 - Nov 2026)  
**Estado:** 🔄 En progreso

---

## 📊 Progreso Actualizado

| Módulo | Estado | Progreso | Última Actualización |
|--------|--------|----------|---------------------|
| 00 - Environment | ✅ | 100% | 2026-03-25 |
| 01 - Fundamentals | 🔄 | 0% | En progreso |
| 02 - Layer 2 | ⬜ | 0% | Pendiente |
| 03 - Layer 3 | ⬜ | 0% | Pendiente |
| 04 - Layer 4 | ⬜ | 0% | Pendiente |
| 04.5 - Traffic Analysis | ⬜ | 0% | Pendiente |
| 05 - Application | ⬜ | 0% | Pendiente |
| 06 - Blue Team | ⬜ | 0% | Pendiente |

---

## 📁 Estructura del Repositorio

| Carpeta | Contenido |
|---------|-----------|
| `00-environment/` | Configuración del laboratorio |
| `01-fundamentals/` | Módulo 1 (Semanas 1-3) |
| `02-layer2/` | Módulo 2 (Semanas 4-6) |
| `03-layer3/` | Módulo 3 (Semanas 7-9) |
| `04-layer4/` | Módulo 4 (Semanas 10-12) |
| `04.5-traffic-analysis/` | Módulo 4.5 (Semanas 13-15) |
| `05-application/` | Módulo 5 (Semanas 16-18) |
| `06-blue-team/` | Módulo 6 (Semanas 19-21) |
| `certifications/` | Evidencia de certificaciones |
| `android-security/` | Fase 3 - Especialización |
| `progress/` | Tracking semanal |

---

## 🎓 Certificaciones

| Certificación | Estado | Fecha Objetivo |
|---------------|--------|----------------|
| Cisco Security Analyst (Principiante) | Iniciado| Abr 2026 |
| Linux Essentials | Iniciado | Abr 2026 |
| Fortinet NSE 1-3 | ⏳ Pendiente | May 2026 |
| Helsinki Cyber Security | ⬜ Pendiente | Ago 2026 |
| ISC2 CC | ⬜ Pendiente | Ago 2026 |
| OWASP MASVS/MASTG | ⬜ Pendiente | Oct 2026 |

---

## 🛠️ Laboratorio

| Máquina | IP | Rol |
|---------|-----|-----|
| Kali Linux | 192.168.100.76 | Atacante |
| Metasploitable 2 | 192.168.100.120 | Víctima |

**Red:** Aislada (192.168.100.0/24) - Sin salida a Internet

---

## 📈 Métricas

| Métrica | Actual | Objetivo |
|---------|--------|----------|
| Ejercicios Python | 1 | 50+ |
| PCAPs documentados | 0 | 10+ |
| Herramientas originales | 0 | 3+ |
| Certificaciones | 0 | 7-8 |
| Labs completados | 1 | 16+ |

---

## ⚠️ Ética

> Todo el conocimiento se usa EXCLUSIVAMENTE en entornos controlados y propios.  
> Nunca en redes de terceros sin autorización escrita.

---

## 📞 Contacto

- **GitHub:** [@guarnieriagustincyber](https://github.com/guarnieriagustincyber)
- **Ubicación:** Resistencia, Chaco, Argentina
- **Estudios:** Licenciatura en Ciberdefensa (FADENA)
